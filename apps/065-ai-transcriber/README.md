...
- 每轮新录音与转写时间码严格从 `00:00` 对齐；已有内容时可用“重新听写”清空旧 take 后立即重录
...
3. 使用“暂停/继续”保留同一录音，或点击“结束”生成本地回放；需要重录时点击“重新听写”，新一轮会从 `00:00` 开始。
...
核心测试覆盖数据清洗、段落排序与去重、编辑/删除、指标、损坏存储恢复、TXT/SRT 和安全文件名。浏览器冒烟测试使用本机 Chrome/Edge DevTools 协议和本地麦克风替身，自动验证首次与重新听写从 `00:00` 开始、录音播放删除、演示入口移除、编辑、搜索、localStorage 恢复、新建清空、桌面与手机布局以及运行时错误。
