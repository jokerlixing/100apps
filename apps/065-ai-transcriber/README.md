...
- 每轮新录音与转写时间码严格从 `00:00` 对齐；已有内容时可用“重新听写”保留稿纸列表，仅重置录音与时间轴后立即重录
- 正在识别的临时文本、最终时间段和实时声带反馈
- 每段直接编辑、删除，按关键词搜索并统计字符、英文词、段落与字速
- 复制带时间的全文，下载 TXT，生成标准 SRT 时间码
- 本地音频导入回放，方便边听边人工校对，并可随时删除当前播放录音
...
- Safari、Firefox 或企业策略禁用 Web Speech API 时，仍可打开已有稿件并使用编辑与导出功能
...
3. 使用“暂停/继续”保留同一录音，或点击“结束”生成本地回放；需要重录时点击“重新听写”，稿纸列表不会被删除，新一轮会从 `00:00` 开始。
4. 本地回放可以下载，也可以点击“删除录音”立即移除播放模块。
5. 在右侧逐段校对文字，搜索或删除不需要的内容。
6. 复制全文、下载 TXT，或导出带时间码的 SRT。
...
核心测试覆盖数据清洗、段落排序与去重、编辑/删除、指标、损坏存储恢复、TXT/SRT 和安全文件名。浏览器冒烟测试使用本机 Chrome/Edge DevTools 协议和本地麦克风替身，自动验证首次与重新听写从 `00:00` 开始、重新听写保留稿纸列表、录音播放删除、演示入口移除、编辑、搜索、localStorage 恢复、新建清空、桌面与手机布局以及运行时错误。
