(function startPortfolio() {
  ...
  function renderBoard() {
    ...
    state.projects.forEach((project) => {
      const button = create('button', 'project-cell');
      button.type = 'button';
      button.dataset.projectId = project.id;
      button.dataset.status = project.status;
      button.setAttribute('role', 'gridcell');
      button.setAttribute('aria-label', `App ${project.code}：${project.name}，${statusLabel(project)}`);
      button.title = `${project.code} · ${project.name}`;
      button.textContent = project.id % 10 === 0 ? project.id : '';
      button.addEventListener('mouseenter', () => selectProject(project.id));
